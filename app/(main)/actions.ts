...
export async function createChat(
  ...
  if (process.env.HELICONE_API_KEY) {
    ...
    options.defaultHeaders = {
      ...
      "Helicone-Property-appname": "zappforge",
      "Helicone-Session-Id": chat.id,
      "Helicone-Session-Name": "zappforge Chat",
    };
  }
  ...
  // Find the first assistant message (the code response)
  const firstAssistantMessage = newChat.messages.find(
    (m) => m.role === "assistant",
  );
  if (firstAssistantMessage) {
    try {
      console.log("Attempting to create GeneratedApp", {
        prompt,
        model,
        code: firstAssistantMessage.content,
        chatId: chat.id,
      });
      await prisma.generatedApp.create({
        data: {
          ...
          code: firstAssistantMessage.content,
          chatId: chat.id,
        } as any, // TEMPORARY: bypass type error
      });
      console.log("GeneratedApp created successfully");
    } catch (err) {
      console.error("Error creating GeneratedApp:", err);
    }
  } else {
    console.warn("No assistant message found for chat", chat.id);
  }
...
export async function getAllGeneratedApps() {
  const prisma = getPrisma();
  return prisma.generatedApp.findMany({
